...
class GeometryService:
    ...
    def load_current_geometry(self, session_id: int, as_site: bool = False) -> Union[Dict[str, Any], Site]:
        ...
        if not current_file.exists():
            ...
            return {
                ...
                "points": [], "segments": [], "geometryLayers": []
            }
        try:
    ...
    def save_geometry(
        ...
        site.history = {"currentVersion": new_version, "previousVersionFile": previous_version_file}
        current_file = self.get_current_geometry_file(session_id)
        try:
    ...
    def add_point(
        ...
        try:
            ...
            new_point = Point(x=x, y=y, layer=point_attributes.get("layer", ""), attributes=point_attributes)
    ...
    def recalculate_arc(
        ...
        new_arc = ArcSegment.create_from_bearing_to_center(
            start_point=start_point, quadrant=quadrant, bearing=bearing, radius=radius,
            length=length, angle=angle, rotation=rotation,
            id=arc_id, layer=segment.layer, attributes=segment.attributes
        )
        for layer in site.geometry_layers:
            for parcel in layer.parcels:
                if parcel.geometry and parcel.geometry.remove_segment(arc_id):
                    ...
                    return cast(Site, self.save_geometry(session_id, site, action="recalculate_arc"))
